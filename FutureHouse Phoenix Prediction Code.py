...
# --- Dependencies for FutureHouse Client ---
# You will need to install this library:
# pip install futurehouse-client
# You will also need a FutureHouse API key.
from futurehouse_client import FutureHouseClient, JobNames
# Note: pathlib and aviary.core imports from the snippet are not needed for this core functionality
# from pathlib import Path
# from aviary.core import DummyEnv
# import ldp
# will ask to type in the API key when running the code
# ---------------------------------------------


class ReactionPredictor:
    def __init__(self):
        # --- Initialize the AI Model (FutureHouse Phoenix API) ---
        self.phoenix_client = None
        self.setup_phoenix_api() # Call the method to set up the Phoenix API client
        # ---------------------------------------------------------
    ...
    def setup_phoenix_api(self):
        """Configures the FutureHouse Phoenix API client using user-provided key."""
        print("Setting up FutureHouse Phoenix API client...")
        # --- Prompt the user for the API key ---
        api_key = input("Please enter your FutureHouse API key: ").strip()
        # ---------------------------------------

        if not api_key:
            print("FutureHouse API key not provided. Phoenix prediction will not be available.")
            self.phoenix_client = None
            return

        try:
            # --- Initialize the FutureHouseClient with the provided API key ---
            self.phoenix_client = FutureHouseClient(api_key=api_key)
            print("FutureHouse Phoenix API client configured successfully.")
        except Exception as e:
            print(f"Error setting up FutureHouse Phoenix API client: {e}")
            print("Phoenix prediction will not be available.")
            self.phoenix_client = None
    ...
    def predict_reaction_phoenix(self, reactants_smiles):
        """Predict reaction products using the FutureHouse Phoenix API."""
        if self.phoenix_client is None:
            print("FutureHouse Phoenix API client not available. Cannot perform prediction.")
            # Fallback to returning the first reactant as product (or empty list)
            return [reactants_smiles[0]] if reactants_smiles else []

        print("Querying FutureHouse Phoenix API...")

        try:
            # --- Use the futurehouse-client to submit a task to the PHOENIX job ---
            reactants_string = " + ".join(reactants_smiles)
            # Craft a query for the Phoenix agent asking for product SMILES
            # Based on the documentation, a clear instruction in the query should work.
            query_text = f"Predict the major chemical reaction product(s) in SMILES format for the reaction between: {reactants_string}. Provide ONLY the product SMILES separated by dots, and nothing else."

            task_data = {
                "name": JobNames.PHOENIX, # Specify the Phoenix job using JobNames enum
                "query": query_text,
                # Add any other required parameters for the Phoenix job if specified in their docs
            }

            # Run the task and wait for completion
            # The run_tasks_until_done method handles polling internally
            task_response = self.phoenix_client.run_tasks_until_done(task_data)

            # --- Parse the response to extract product SMILES ---
            # The structure of task_response will depend on the Phoenix job's output format.
            # Assuming the main answer is in task_response.answer
            predicted_smiles_string = task_response.answer.strip() if hasattr(task_response, 'answer') and task_response.answer else ""
            ...
            print(f"Phoenix predicted raw output: {predicted_smiles_string}")
            ...
            for smiles in products_smiles:
                 try:
                     ...
                     if mol:
                         ...
                         print(f"Invalid SMILES predicted by Phoenix: '{smiles}'. Skipping.")
                 except:
                     print(f"Error parsing SMILES predicted by Phoenix: '{smiles}'. Skipping.")
                     continue

            if not valid_products:
                 print("No valid product SMILES found from Phoenix prediction. Generating default product.")
            ...
            print(f"Error during FutureHouse Phoenix API prediction: {e}")
            # Fallback to returning the first reactant as product
            return [reactants_smiles[0]] if reactants_smiles else []
    ...
    def run_prediction_and_simulation(self, reactant_names, condition, temperature, simulation_time, custom_k=None, user_product_input=None):
        ...
        # --- Determine products: Use user input if provided, otherwise predict with Phoenix ---
        products_smiles = []
        product_source = "Predicted by Phoenix API" # Default source
        ...
        if user_product_input and user_product_input.strip():
            ...
            print("\nPredicting reaction products using FutureHouse Phoenix API...")
            # Call the Phoenix prediction method
            products_smiles = self.predict_reaction_phoenix(reactants_smiles)

            if not products_smiles:
                print("Phoenix API prediction failed or returned no valid products. Cannot proceed.")
...
def main():
    ...
    # Check if Phoenix API client loaded successfully (placeholder check)
    if predictor.phoenix_client is None:
        print("\nWarning: FutureHouse Phoenix API client failed to set up. Product prediction via API will not be available.")
        print("You must provide product names manually for each reaction.")


    print("\n=== Chemical Reaction Prediction and Simulation System ===")
    print("This program predicts reaction pathways (via Phoenix API) or uses provided products,")
    ...
    while True:
        ...
        # Adjust prompt based on whether Phoenix API is available
        product_prompt = "\nEnter product names (separated by comma, optional - press Enter to predict with Phoenix API), or 'done' to exit: "
        if predictor.phoenix_client is None:
             product_prompt = "\nEnter product names (separated by comma, REQUIRED if no Phoenix API access - press Enter to skip), or 'done' to exit: "
