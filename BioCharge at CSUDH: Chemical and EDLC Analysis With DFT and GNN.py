...
import subprocess
import sys
import difflib  # V5.0: Added for fuzzy string matching
warnings.filterwarnings('ignore')

# ==================== V5.0: LOGGING SETUP ====================
# Setup comprehensive logging for debugging and analysis
logging.basicConfig(
    filename='biocharge_v5_log.txt',
    level=logging.INFO,
...
# === V5.0: Enhanced Chemistry Libraries ===
try:
...
# ==================== V5.0: NEW - Graph Neural Network Dependencies ====================
...
try:
    ...
    print("⚠️ PyTorch not available. GNN features disabled.")
...
if TORCH_AVAILABLE:
    try:
        ...
        print("✅ PyTorch Geometric loaded - GNN capabilities enabled")
    except ImportError:
        TORCH_GEOMETRIC_AVAILABLE = False
...
# ==================== V5.0: DFT CALCULATOR MODULE ====================

class DFTCalculator:
    """
    V5.0: Density Functional Theory Calculator
    Interfaces with ORCA, xTB, and Gaussian for quantum chemical calculations
    """
    
    def __init__(self, method='xtb', scratch_dir='./dft_scratch'):
        """
        Initialize DFT calculator
        
        Args:
            method: 'xtb' (fast, semi-empirical), 'orca' (DFT), or 'gaussian' (high-accuracy)
            scratch_dir: Directory for temporary calculation files
        """
        ...
        self.xtb_available = self._check_executable('xtb')
        self.orca_available = self._check_executable('orca')
        self.gaussian_available = self._check_executable('g16') or self._check_executable('g09')
        
        if self.method == 'xtb' and not self.xtb_available:
            logging.warning("xTB not found. Install from: https://github.com/grimme-lab/xtb")
        elif self.method == 'orca' and not self.orca_available:
            logging.warning("ORCA not found. Install from: https://orcaforum.kofo.mpg.de")
        elif self.method == 'gaussian' and not self.gaussian_available:
            logging.warning("Gaussian not found. Commercial software required.")
            
        print(f"🔬 DFT Calculator initialized (method: {self.method})")
        print(f"   xTB: {'✅' if self.xtb_available else '❌'}")
        print(f"   ORCA: {'✅' if self.orca_available else '❌'}")
        print(f"   Gaussian: {'✅' if self.gaussian_available else '❌'}")
    
    def _check_executable(self, name):
        ...
        try:
            result = subprocess.run([name, '--version'], 
                                   ...
                                   timeout=5,
                                   text=True)
            return result.returncode == 0 or 'version' in result.stdout.lower() or 'version' in result.stderr.lower()
        except (subprocess.TimeoutExpired, FileNotFoundError, Exception):
            return False
    
    def smiles_to_xyz(self, smiles, optimize_3d=True):
        ...
        try:
            ...
            xyz_lines = [f"{len(atoms)}", "Generated from SMILES"]
            ...
            logging.error(f"SMILES to XYZ conversion failed: {e}")
            return None
    
    def calculate_properties(self, smiles, properties=['energy', 'homo', 'lumo', 'dipole']):
        """
        Calculate molecular properties using DFT
        ...
        Returns:
            dict: Calculated properties or None
        """
        if not any([self.xtb_available, self.orca_available, self.gaussian_available]):
            logging.warning("No DFT software available, returning estimated properties")
            return self._estimate_properties(smiles)
        
        # Generate XYZ coordinates
        xyz_content = self.smiles_to_xyz(smiles)
        if xyz_content is None:
            return self._estimate_properties(smiles)
        
        # Run calculation based on method
        if self.method == 'xtb' and self.xtb_available:
            return self._run_xtb(xyz_content, properties)
        elif self.method == 'orca' and self.orca_available:
            return self._run_orca(xyz_content, properties)
        elif self.method == 'gaussian' and self.gaussian_available:
            return self._run_gaussian(xyz_content, properties)
        else:
            return self._estimate_properties(smiles)
    
    def _run_xtb(self, xyz_content, properties):
        """Run xTB calculation"""
        try:
            # Create temporary input file
            input_file = self.scratch_dir / f"mol_{int(time.time())}_{os.getpid()}.xyz"
            with open(input_file, 'w') as f:
                f.write(xyz_content)
            
            # Run xTB
            cmd = ['xtb', str(input_file), '--opt', '--gfn', '2']
            ...
            for line in output.split('\n'):
                if 'TOTAL ENERGY' in line:
                    try:
                        props['energy'] = float(line.split()[-3])
                    except:
                        pass
                elif 'HOMO-LUMO GAP' in line:
                    try:
                        props['gap'] = float(line.split()[-2])
                    except:
                        pass
                elif '(HOMO)' in line:
                    try:
                        props['homo'] = float(line.split()[-2])
                    except:
                        pass
                elif '(LUMO)' in line:
                    try:
                        props['lumo'] = float(line.split()[-2])
                    except:
                        pass
                elif 'molecular dipole' in line:
                    try:
                        props['dipole'] = float(line.split()[-2])
                    except:
                        pass
            ...
            try:
                input_file.unlink()
                for f in self.scratch_dir.glob(f"{input_file.stem}*"):
                    f.unlink()
            except:
                pass
            
            if props:
                logging.info(f"xTB calculation completed: {len(props)} properties")
            return props if props else None
            
        except Exception as e:
            logging.error(f"xTB calculation failed: {e}")
            return None
    
    def _run_orca(self, xyz_content, properties):
        """Run ORCA calculation"""
        try:
            # Create ORCA input file
            input_file = self.scratch_dir / f"mol_{int(time.time())}_{os.getpid()}.inp"
            
            # Simple ORCA input for fast calculation
            orca_input = f"""! B3LYP def2-SVP OPT
%maxcore 2000

* xyz 0 1
{chr(10).join(xyz_content.split(chr(10))[2:])}  
*
"""
            ...
            for line in output.split('\n'):
                if 'FINAL SINGLE POINT ENERGY' in line:
                    try:
                        props['energy'] = float(line.split()[-1])
                    except:
                        pass
            ...
            if props:
                logging.info(f"ORCA calculation completed: {len(props)} properties")
            return props if props else None
            
        except Exception as e:
            logging.error(f"ORCA calculation failed: {e}")
            return None
    
    def _run_gaussian(self, xyz_content, properties):
        """Run Gaussian calculation"""
        try:
            # Create Gaussian input file
            input_file = self.scratch_dir / f"mol_{int(time.time())}_{os.getpid()}.gjf"
            
            gaussian_input = f"""%chk={input_file.stem}.chk
%mem=2GB
%nprocshared=2
# B3LYP/6-31G(d) Opt
...
"""
            ...
            # Run Gaussian (g16 or g09)
            cmd = ['g16', str(input_file)] if self._check_executable('g16') else ['g09', str(input_file)]
            result = subprocess.run(cmd,
                                   capture_output=True,
                                   text=True,
                                   timeout=900,
            ...
            for line in output.split('\n'):
                if 'SCF Done' in line:
                    try:
                        props['energy'] = float(line.split()[4])
                    except:
                        pass
            ...
            if props:
                logging.info(f"Gaussian calculation completed: {len(props)} properties")
            return props if props else None
            
        except Exception as e:
            logging.error(f"Gaussian calculation failed: {e}")
            return None
    
    def _estimate_properties(self, smiles):
        """Fallback: Estimate properties using RDKit descriptors"""
        try:
            mol = Chem.MolFromSmiles(smiles)
            if mol is None:
                return None
            
            # Use RDKit descriptors as proxies
            props = {
                'energy': -Descriptors.MolWt(mol) * 0.01,  # Rough estimate
                'homo': -5.0 - Descriptors.NumHeteroatoms(mol) * 0.1,  # Rough estimate
                'lumo': -2.0 + Descriptors.NumAromaticRings(mol) * 0.2,  # Rough estimate
                'gap': 3.0 + Descriptors.NumAromaticRings(mol) * 0.1,
                'dipole': Descriptors.TPSA(mol) * 0.01,
                'estimated': True  # Flag that these are estimates
            }
            
            logging.info("Using RDKit estimated properties (no DFT software available)")
            return props
            
        except Exception as e:
            logging.error(f"Property estimation failed: {e}")
            return None


# ==================== V5.0: GRAPH NEURAL NETWORK MODULE ====================

if TORCH_GEOMETRIC_AVAILABLE:
    class MolecularGNN(nn.Module):
        """
        V5.0: Graph Neural Network for direct property prediction from molecular graphs
        Architecture inspired by ChemProp with attention mechanisms
        """
        
        def __init__(self, node_features=44, edge_features=12, hidden_dim=256, 
                     num_layers=4, output_dim=1, dropout=0.2):
    ...
    # Dummy class if PyTorch Geometric not available
    class MolecularGNN:
...
class GNNPredictor:
    """
    V5.0: GNN-based predictor for capacitance and electrochemical properties
    """
    ...
    def __init__(self, model_path=None, device='cpu'):
        ...
        self.capacitance_model = MolecularGNN(output_dim=1).to(self.device)
        self.conductivity_model = MolecularGNN(output_dim=1).to(self.device)
        self.stability_model = MolecularGNN(output_dim=1).to(self.device)
        
        # Load pretrained weights if available
        if model_path and Path(model_path).exists():
            self.load_models(model_path)
        else:
            logging.info("No pretrained GNN model found. Using randomly initialized weights.")
        
        print(f"🧠 GNN Predictor initialized on {self.device}")
    ...
    def smiles_to_graph(self, smiles):
        ...
        try:
            ...
            for atom in mol.GetAtoms():
                ...
                # One-hot encoding for common elements in biochar
                element_types = ['C', 'H', 'O', 'N', 'S', 'P', 'F', 'Cl', 'Br']
                element_onehot = [int(atom.GetSymbol() == e) for e in element_types]
                ...
                node_features.append(features[:44])
            ...
            logging.error(f"SMILES to graph conversion failed: {e}")
            return None
    
    def predict(self, smiles, property_type='capacitance'):
        """
        Predict molecular property using GNN
        
        Args:
            smiles: SMILES string
            property_type: 'capacitance', 'conductivity', or 'stability'
        ...
        try:
            ...
            if property_type == 'capacitance':
                model = self.capacitance_model
            elif property_type == 'conductivity':
                model = self.conductivity_model
    ...
    def predict_all_properties(self, smiles):
        """Predict all properties at once"""
        if not self.available:
            return None
            
        return {
            'gnn_capacitance': self.predict(smiles, 'capacitance'),
            'gnn_conductivity': self.predict(smiles, 'conductivity'),
            'gnn_stability': self.predict(smiles, 'stability')
        }
    
    def train_on_data(self, smiles_list, values, property_type='capacitance', epochs=100):
        """
        Train GNN on experimental data
        
        Args:
            smiles_list: List of SMILES strings
            values: List of experimental values
            property_type: Property to train on
        ...
        try:
            ...
            for smi, val in zip(smiles_list, values):
                ...
                if graph is not None and val is not None:
                    graph.y = torch.tensor([[float(val)]], dtype=torch.float)
                    graphs.append(graph)
                    valid_values.append(val)
            
            if len(graphs) < 3:
                logging.warning(f"Insufficient data for training: {len(graphs)} samples")
                return False
            ...
            print(f"\n🧠 Training GNN on {len(graphs)} samples for {epochs} epochs...")
    ...
    def save_models(self, path='gnn_models'):
        """Save trained models"""
        if not self.available:
            return
        ...
        torch.save(self.capacitance_model.state_dict(), save_dir / 'capacitance_model.pt')
        torch.save(self.conductivity_model.state_dict(), save_dir / 'conductivity_model.pt')
        torch.save(self.stability_model.state_dict(), save_dir / 'stability_model.pt')
        
        print(f"✅ GNN models saved to {save_dir}")
    
    def load_models(self, path='gnn_models'):
        ...
        try:
            self.capacitance_model.load_state_dict(torch.load(load_dir / 'capacitance_model.pt', map_location=self.device))
            self.conductivity_model.load_state_dict(torch.load(load_dir / 'conductivity_model.pt', map_location=self.device))
            self.stability_model.load_state_dict(torch.load(load_dir / 'stability_model.pt', map_location=self.device))
            print(f"✅ GNN models loaded from {load_dir}")
        except Exception as e:
            logging.warning(f"Could not load GNN models: {e}")
...
# ==================== V5.0: NEW ENHANCED CLASSES ====================

class ElectrochemicalScorer:
    """V5.0: Real-world electrochemical scoring using ChemLib
...
class ChemicalSpaceMapper:
    """V5.0: Interactive chemical space visualization using ChemPlot
...
class ThermodynamicFeasibilityAnalyzer:
    """V5.0: Thermodynamic analysis using ChemPy principles
...
class CapacitancePotentialScorer:
    ...
    def enhanced_score_with_electrochemistry(self, 
        ...
        """V5.0: Enhanced scoring combining structural + electrochemical properties
        ...
        # Add electrochemical analysis (V5.0 NEW)
        if CHEMLIB_ELECTRO:
...
class EnhancedReactionPredictor:
    ...
    def __init__(self):
        ...
        # V5.0: NEW - DFT and GNN Integration
        self.dft_calculator = None
        self.gnn_predictor = None
        try:
            self.dft_calculator = DFTCalculator(method='xtb')
            logging.info("DFT Calculator initialized")
        except Exception as e:
            logging.warning(f"DFT Calculator not initialized: {e}")
        
        try:
            if TORCH_GEOMETRIC_AVAILABLE:
                self.gnn_predictor = GNNPredictor(device='cpu')
                logging.info("GNN Predictor initialized")
        except Exception as e:
            logging.warning(f"GNN Predictor not initialized: {e}")
    ...
    def run_interactive_analysis(self):
        ...
        # FIXED V5.0: Calculate and store EDLC potential score for interactive analysis
    ...
    def predict_with_dft_gnn(self, reactants_smiles, product_smiles=None):
        """
        V5.0: Enhanced prediction using DFT and GNN
        
        Args:
            reactants_smiles: Reactants SMILES string
            product_smiles: Optional product SMILES (if known)
        ...
        # Get base transformer predictions
        if product_smiles:
            result['product_smiles'] = product_smiles
        else:
            # Use existing consensus prediction
            pred = self.consensus_system.predict(reactants_smiles)
            if pred and 'product' in pred:
                result['product_smiles'] = pred['product']
            else:
                return None
        
        # Add DFT calculations
        if self.dft_calculator and result.get('product_smiles'):
            try:
                dft_props = self.dft_calculator.calculate_properties(result['product_smiles'])
                if dft_props:
                    result['dft_properties'] = dft_props
                    logging.info("DFT properties calculated")
            except Exception as e:
                logging.error(f"DFT calculation failed: {e}")
        
        # Add GNN predictions
        if self.gnn_predictor and self.gnn_predictor.available and result.get('product_smiles'):
            try:
                gnn_props = self.gnn_predictor.predict_all_properties(result['product_smiles'])
                if gnn_props:
                    result['gnn_predictions'] = gnn_props
                    logging.info("GNN predictions completed")
            except Exception as e:
                logging.error(f"GNN prediction failed: {e}")
        
        # Add electrochemical scoring
        try:
            ec_score = self.capacitance_scorer.score_molecule(result['product_smiles'])
            if ec_score:
                result['ec_score'] = ec_score
        except Exception as e:
            logging.error(f"EC scoring failed: {e}")
        
        return result
    
    def run_dft_standalone(self, smiles):
        """V5.0: Run standalone DFT calculation"""
        if not self.dft_calculator:
            return None
        return self.dft_calculator.calculate_properties(smiles)
    
    def run_gnn_standalone(self, smiles):
        """V5.0: Run standalone GNN prediction"""
        if not self.gnn_predictor or not self.gnn_predictor.available:
            return None
        return self.gnn_predictor.predict_all_properties(smiles)
    
    def train_gnn_model(self, smiles_list, values, property_type='capacitance', epochs=100):
        """V5.0: Train GNN on experimental data"""
        if not self.gnn_predictor or not self.gnn_predictor.available:
            print("❌ GNN not available")
            return False
        return self.gnn_predictor.train_on_data(smiles_list, values, property_type, epochs)
...
class EnhancedBatchProcessor:
    ...
    def process_single_reaction(self, row):
        ...
        try:
            ...
            # V5.0: Add DFT and GNN predictions for product
            if products and len(products) > 0:
                primary_product = products[0]  # Use first product
                
                # DFT calculations
                if self.predictor.dft_calculator:
                    try:
                        dft_props = self.predictor.dft_calculator.calculate_properties(primary_product)
                        if dft_props:
                            result['DFT_Energy'] = dft_props.get('energy')
                            result['DFT_HOMO'] = dft_props.get('homo')
                            result['DFT_LUMO'] = dft_props.get('lumo')
                            result['DFT_Gap'] = dft_props.get('gap')
                            result['DFT_Dipole'] = dft_props.get('dipole')
                            result['DFT_Estimated'] = dft_props.get('estimated', False)
                            logging.info(f"DFT calculated for {reaction_id}")
                    except Exception as e:
                        logging.warning(f"DFT failed for {reaction_id}: {e}")
                        result['DFT_Energy'] = None
                        result['DFT_HOMO'] = None
                        result['DFT_LUMO'] = None
                        result['DFT_Gap'] = None
                
                # GNN predictions
                if self.predictor.gnn_predictor and self.predictor.gnn_predictor.available:
                    try:
                        gnn_props = self.predictor.gnn_predictor.predict_all_properties(primary_product)
                        if gnn_props:
                            result['GNN_Capacitance'] = gnn_props.get('gnn_capacitance')
                            result['GNN_Conductivity'] = gnn_props.get('gnn_conductivity')
                            result['GNN_Stability'] = gnn_props.get('gnn_stability')
                            logging.info(f"GNN predictions for {reaction_id}")
                    except Exception as e:
                        logging.warning(f"GNN failed for {reaction_id}: {e}")
                        result['GNN_Capacitance'] = None
                        result['GNN_Conductivity'] = None
                        result['GNN_Stability'] = None
...
def main():
    ...
    print("\n" + "="*70)
    print(" 🔬 BIOCHARGE CHEMISTRY CODE V5.0 - COMPLETE PLATFORM")
    print(" BioCharge at CSUDH - Biochar Supercapacitor Research")
    print(" ✨ High-Throughput Platform V5.0 - ChemLib+ChemPlot Integration")
    print("="*70)
    ...
    while True:
        ...
        print("9. Chemical Space Analysis (V5.0 NEW)")
        print("11. DFT Calculations (V5.0 NEW)")
        print("12. GNN Property Prediction (V5.0 NEW)")
        print("13. Train GNN on Your Data (V5.0 NEW)")
        print("14. System Status & Capabilities (V5.0)")
        print("15. Exit")
        ...
        if choice == "1":
            ...
            print("3. Electrochemical Metal Analysis (V5.0)")
            ...
            if sub_choice == "1":
                ...
                if not png_files:
                    ...
                    print(f"file:///{png_files[0].absolute()}")
        
        
        elif choice == "11":
            # V5.0: DFT Calculations
            print("\n🔬 DFT QUANTUM CHEMISTRY CALCULATIONS")
            print("="*60)
            
            if not predictor.dft_calculator:
                print("❌ DFT Calculator not available")
                print("   Install xTB, ORCA, or Gaussian for DFT calculations")
                continue
            ...
            print("\n⚙️  Running DFT calculation...")
            print("   (This may take 30 seconds to 5 minutes)\n")
            
            props = predictor.run_dft_standalone(smiles)
            
            if props:
                print("\n✅ DFT CALCULATION RESULTS:")
                print("="*60)
                if 'energy' in props:
                    print(f"   • Total Energy: {props['energy']:.6f} Hartree")
                if 'homo' in props:
                    print(f"   • HOMO Energy: {props['homo']:.4f} eV")
                if 'lumo' in props:
                    print(f"   • LUMO Energy: {props['lumo']:.4f} eV")
                if 'gap' in props:
                    print(f"   • HOMO-LUMO Gap: {props['gap']:.4f} eV")
                if 'dipole' in props:
                    print(f"   • Dipole Moment: {props['dipole']:.4f} Debye")
                if 'estimated' in props:
                    print("\n   ⚠️  Note: These are RDKit estimates")
                    print("      Install xTB for accurate quantum calculations")
                print("="*60)
            else:
                print("❌ DFT calculation failed")
        
        elif choice == "12":
            # V5.0: GNN Predictions
            print("\n🧠 GRAPH NEURAL NETWORK PROPERTY PREDICTION")
            print("="*60)
            
            if not predictor.gnn_predictor or not predictor.gnn_predictor.available:
                print("❌ GNN not available")
                print("   Install: pip install torch torch-geometric --break-system-packages")
                continue
            ...
            print("\n⚙️  Running GNN prediction...\n")
            
            props = predictor.run_gnn_standalone(smiles)
            
            if props:
                print("✅ GNN PREDICTIONS:")
                print("="*60)
                if props.get('gnn_capacitance'):
                    print(f"   • Predicted Capacitance: {props['gnn_capacitance']:.2f} F/g")
                if props.get('gnn_conductivity'):
                    print(f"   • Predicted Conductivity: {props['gnn_conductivity']:.2f} S/cm")
                if props.get('gnn_stability'):
                    print(f"   • Cycling Stability Score: {props['gnn_stability']:.2f}")
                print("\n   💡 Note: Predictions improve when trained on your data!")
                print("      Use Option 13 to train with experimental results")
                print("="*60)
            else:
                print("❌ GNN prediction failed")
        
        elif choice == "13":
            # V5.0: Train GNN
            print("\n🧠 TRAIN GNN ON YOUR EXPERIMENTAL DATA")
            print("="*60)
            ...
            if not results_files:
                ...
                print("   Run batch processing first (Option 2) to generate data")
                continue
            ...
            try:
                ...
                print("\nWhat property to train on?")
                print("1. Capacitance Score (from predictions)")
                print("2. Experimental Capacitance (if you have measurements)")
                
                prop_choice = input("Select (1-2): ").strip()
                
                if prop_choice == "1":
                    if 'Capacitance_Score' in df.columns and 'Product_SMILES' in df.columns:
                        smiles_list = df['Product_SMILES'].dropna().tolist()
                        values = df['Capacitance_Score'].dropna().tolist()
                        property_type = 'capacitance'
                    else:
                        print("❌ Required columns not found")
                        continue
                elif prop_choice == "2":
                    if 'Experimental_Capacitance' in df.columns and 'Product_SMILES' in df.columns:
                        df_clean = df.dropna(subset=['Experimental_Capacitance', 'Product_SMILES'])
                        smiles_list = df_clean['Product_SMILES'].tolist()
                        values = df_clean['Experimental_Capacitance'].tolist()
                        property_type = 'capacitance'
                    else:
                        print("❌ No experimental data found")
                        print("   Add 'Experimental_Capacitance' column to your Excel file")
                        continue
                else:
                ...
                valid_data = [(s, v) for s, v in zip(smiles_list, values) 
                             if s and v is not None and Chem.MolFromSmiles(str(s)) is not None]
                
                if len(valid_data) < 5:
                    print(f"❌ Need at least 5 valid samples (found {len(valid_data)})")
                    continue
                
                smiles_list, values = zip(*valid_data)
                
                print(f"\n🧠 Training GNN on {len(smiles_list)} samples...")
                epochs = int(input("Number of training epochs (default 100): ").strip() or "100")
                
                success = predictor.train_gnn_model(
                    list(smiles_list), 
                    list(values), 
                    property_type, 
                    epochs
                ...
                if success:
                    # Save trained model
                    if predictor.gnn_predictor:
                        predictor.gnn_predictor.save_models('trained_gnn_models')
                    print("\n✅ Training complete! Model saved to 'trained_gnn_models/'")
                    print("   Future predictions will use your trained model!")
            ...
            print("\n📦 Core Dependencies:")
            print(f"   • ChemPy: {'✅' if CHEMPY_AVAILABLE else '❌'}")
            print(f"   • Transformers: {'✅' if TRANSFORMERS_AVAILABLE else '❌'}")
            print(f"   • RXNMapper: {'✅' if RXNMAPPER_AVAILABLE else '❌'}")
            print(f"   • ChemPlot: {'✅' if CHEMPLOT_AVAILABLE else '❌'}")
            ...
            if TORCH_GEOMETRIC_AVAILABLE:
                print(f"      ✅ Available")
                if predictor.gnn_predictor and predictor.gnn_predictor.available:
                    print(f"      Device: {predictor.gnn_predictor.device}")
            else:
                print("      ❌ Not available")
                print("      Install: pip install torch torch-geometric --break-system-packages")
            
            print("\n   • DFT Calculator:")
            if predictor.dft_calculator:
                print(f"      ✅ Initialized (method: {predictor.dft_calculator.method})")
                print(f"      xTB: {'✅' if predictor.dft_calculator.xtb_available else '❌'}")
                print(f"      ORCA: {'✅' if predictor.dft_calculator.orca_available else '❌'}")
                print(f"      Gaussian: {'✅' if predictor.dft_calculator.gaussian_available else '❌'}")
                
                if not any([predictor.dft_calculator.xtb_available, 
                           predictor.dft_calculator.orca_available,
                           predictor.dft_calculator.gaussian_available]):
                    print("\n      ⚠️  No DFT software found - using RDKit estimates")
            ...
            for model_name, model_info in predictor.available_models.items():
                status = "✅ Loaded" if model_info.get('loaded') else "⚙️  Available"
                print(f"   • {model_name}: {status}")
            
            print("\n💾 Trained Models:")
            gnn_model_dir = Path('trained_gnn_models')
            if gnn_model_dir.exists() and list(gnn_model_dir.glob('*.pt')):
                print(f"   ✅ Trained GNN models found in: {gnn_model_dir}")
                for model_file in gnn_model_dir.glob('*.pt'):
                    print(f"      • {model_file.name}")
            else:
                print("   ⚙️  No trained GNN models (train with Option 13)")
            
            print("\n💡 Installation Commands:")
            print("   pip install torch torch-geometric --break-system-packages")
            print("   conda install -c conda-forge xtb")
            print("="*60)
        
        elif choice == "15":
        # OLD Option 10 - now removed
        # elif choice == "10":
            print("\n👋 Thank you for using the Enhanced Chemistry System V5.0")
            print("   🎯 Batch Processing Repaired - BioCharge at CSUDH")
