#individual reaction rate graph x-axis be minutes instead of hours, corresponds to user simulation time input
#Chemplot,ChemPy, Chemlib, DeepChem, etc libraries for chemical data analysis and visualization
#use bar graph instead of heatmap
#allow user to type canonical SMILES if not found in local database or in PubChem
#even parsing of each reation and memory wipe after each reaction for memory exhaustion problem in bactch processing
#exponential decay of reaction rate constants over time for more accurate simulation
...
class ReactionT5v2Model:
    ...
    def predict(self, reactants_smiles: List[str], condition: str = None) -> List[str]:
        ...
        try:
            ...
            with torch.no_grad():
                # V3.3 FIX: Removed temperature parameter to fix warning
...
class PredictionConfidenceScorer:
    ...
    def calculate_confidence(self, reaction_id: str) -> Dict:
        ...
        # V3.3 FIX: Check if all predictions are empty
...
class EnhancedPubChemAPI:
    ...
    def __init__(self, cache_dir="pubchem_cache"):
        ...
        # V3.3: EXPANDED DATABASE with Feedstock compounds not found in PubChem
        self.local_database = {
            # Biomass components
...
class BatchAnalysisVisualizer:
    ...
    def create_cumulative_rate_scatter(results_df: pd.DataFrame, 
        ...
        """V3.3 NEW: Create comprehensive scatter plot with ALL reactions on one graph"""
...
class EnhancedReactionPredictor:
    """Main predictor class with V3.3 fixes"""
    ...
    def initialize_enhanced_models(self):
        ...
        print("\n🚀 Initializing Enhanced Chemistry System V3.3...")
        ...
        print("✅ Enhanced models initialized (V3.3 - Batch Processing Repaired)")
    ...
    def predict_reaction(self, reactants_smiles, condition=None, reaction_id=None):
        """V3.3 Enhanced reaction prediction with proper failure detection"""
        
        # V3.3 FIX: Check for invalid inputs at the start
        ...
        # V3.3 FIX: Remove duplicates and validate products are different from reactants
        unique_products = []
        seen = set()
        for p in products:
        ...
        # V3.3 FIX: If no valid products, return explicit failure
    ...
    def run_interactive_analysis(self):
        """Enhanced interactive single reaction analysis with V3.3 features"""
        
        print("\n" + "="*60)
        print("🧪 ENHANCED SINGLE REACTION ANALYSIS V3.3")
        print("="*60)
        ...
        # V3.3 NEW: Ask if user wants individual rate graph
        ...
        for name in reactant_names:
            ...
            if smiles:
                ...
                # Check for metal
                if any(m in smiles for m in ['[Ni]', '[Cu]', '[Fe]', '[Co]', '[Pd]', '[Al]']):
                    metal_present = name
        ...
        # V3.3 FIX: Handle failure cases properly
        ...
        # V3.3 NEW: Print balanced chemical equation
        ...
        # V3.3 NEW: Generate individual rate graph if requested
        if generate_graph:
            print("\n📊 Generating individual reaction rate graph...")
            try:
                # Generate time points
                time_points = np.linspace(0, sim_time_s, 100)
                C0 = initial_conc
                concentrations = C0 * np.exp(-k * time_points)
                conversions_curve = (1 - concentrations / C0) * 100
                
                fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
                
                # Left plot: Concentration vs Time
                ax1.plot(time_points / 3600, concentrations, 'b-', linewidth=2.5)
                ax1.set_xlabel('Time (hours)', fontsize=13, fontweight='bold')
                ax1.set_ylabel('Concentration (mol/L)', fontsize=13, fontweight='bold')
                ax1.set_title(f'Concentration Decay\n{" + ".join(reactant_names)}', fontsize=15, fontweight='bold')
                ax1.grid(True, alpha=0.3)
                ax1.set_ylim([0, C0 * 1.1])
                
                # Add half-life line
                if half_life != float('inf') and half_life < sim_time_s:
                    ax1.axvline(half_life / 3600, color='r', linestyle='--', linewidth=2,
                               label=f't½ = {half_life/3600:.2f} h')
                    ax1.legend(fontsize=11)
                
                # Right plot: Conversion vs Time
                ax2.plot(time_points / 3600, conversions_curve, 'g-', linewidth=2.5)
                ax2.set_xlabel('Time (hours)', fontsize=13, fontweight='bold')
                ax2.set_ylabel('Conversion (%)', fontsize=13, fontweight='bold')
                ax2.set_title(f'Conversion Progress\n{condition.capitalize()} Conditions', fontsize=15, fontweight='bold')
                ax2.grid(True, alpha=0.3)
                ax2.set_ylim([0, 105])
                
                # Add final conversion marker
                ax2.axhline(conversion, color='r', linestyle='--', linewidth=2,
                           label=f'Final: {conversion:.1f}%')
                ax2.legend(fontsize=11)
                
                # Add reaction info at bottom
                info_text = f"T = {temperature-273.15:.0f}°C  |  k = {k:.2e} s⁻¹  |  Ea = {params['Ea']/1000:.1f} kJ/mol"
                ...
                # Save
                graph_filename = f"interactive_{reaction_id}_rate_graph.png"
                plt.savefig(graph_filename, dpi=300, bbox_inches='tight')
                plt.show()
                
                print(f"✅ Saved rate graph: {graph_filename}")
...
class EnhancedBatchProcessor:
    """Enhanced batch processor with V3.3 Improvements"""
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("📊 ENHANCED BATCH PROCESSING V3.3")
        print("="*60)
    ...
    def create_template(self):
        ...
        # V3.3: Template with Ailanthus compounds
        template_data = {
    ...
    def process_excel(self):
        """Process Excel file with V3.3 FIXED enhancements"""
        
        # V3.3 FIX: Smart file finding
        ...
        # V3.3 NEW: Create cumulative scatter plot for ALL reactions
    ...
    def process_single_reaction(self, row):
        """V3.3: Process single reaction with consistent result structure"""
        
        result = {'ID': row.get('ID', f"RXN_{time.time()}")}
        start_time = time.time()
        
        # V3.3 CRITICAL FIX: Initialize ALL fields with default values for consistency
        default_result = {
            'Reactants': 'Unknown',
        ...
        try:
            ...
            for i in range(1, 4):
                ...
                if col in row and pd.notna(row[col]):
                    ...
                    if smiles:
                        ...
                        # Check for metal
                        if any(m in smiles for m in ['[Ni]', '[Cu]', '[Fe]', '[Co]', '[Pd]', '[Al]', '[Zn]']):
                            metal_present = name
            ...
            # V3.3 FIX: Handle prediction results consistently
            if isinstance(products_result, dict):
                products = products_result.get('products', [])
                confidence = products_result.get('confidence', 0)
            ...
            # V3.3 FIX: Check if prediction failed, but DON'T return early
            ...
            # V3.3: Generate stoichiometric equation
            try:
                balancer = ChemicalEquationBalancer()
    ...
    def visualize_results(self):
        ...
        # V3.3: Create all visualizations
...
def main():
    """Main program entry point with V3.3"""
    
    print("\n" + "="*70)
    print(" 🧪 ENHANCED CHEMICAL REACTION & EDLC ANALYSIS SYSTEM V3.3")
    print(" BioCharge at CSUDH - Biochar Supercapacitor Research")
    print(" ✨ High-Throughput Platform V3.3 - Batch Processing Repaired")
    print("="*70)
        ...
        if choice == "1":
            ...
            if sub_choice == "1":
                ...
                if results_files:
                    ...
                    # V3.3: Create cumulative scatter plot
            ...
            print("\n📚 HELP - Version 3.3 Features:")
            print("\n🎯 WHAT'S FIXED IN V3.3:")
            print("✅ BATCH PROCESSING NOW WORKS! Consistent result dictionary structure")
            print("✅ All reactions return same fields (success or failure)")
            print("✅ No more pandas DataFrame construction errors")
            print("✅ Fixed temperature warning in ReactionT5v2")
            print("✅ Feedstock compounds fully documented (see EnhancedPubChemAPI)")
            print("✅ No false successes: System properly reports failures")
            print("✅ Stoichiometric equations in batch results")
            print("✅ Cumulative scatter: All reactions on one graph")
            ...
            print("See EnhancedPubChemAPI class docstring for full documentation!")
            ...
            print("• Includes stoichiometric equations in results")
            print("• Creates ONE cumulative scatter plot for all reactions")
            
        elif choice == "9":
            print("\n👋 Thank you for using the Enhanced Chemistry System V3.3")
