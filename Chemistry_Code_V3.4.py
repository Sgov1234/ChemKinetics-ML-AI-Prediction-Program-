...
class MetalSelectivityQuantifier:
    ...
    def plot_selectivity_bar_chart(self, save_path: str = "metal_selectivity.png"):
        ...
        for compound_type, metal_data in self.compound_types.items():
...
class EnhancedReactionPredictor:
    ...
    def get_smiles_from_compound(self, compound_name):
        """Get SMILES using enhanced PubChem API with V3.4 manual input fallback"""
        ...
        # V3.4: If not found, offer manual SMILES input
        if not smiles:
    ...
    def run_interactive_analysis(self):
        ...
        # V3.4 NEW: Generate individual rate graph if requested
        if generate_graph:
            print("\n📊 Generating individual reaction rate graph...")
            try:
                # V3.4: Use exponential decay kinetics model
                C0 = initial_conc
                time_points, concentrations = self.decay_kinetics.calculate_conversion_with_decay(
                    k_initial=k,
                    time_total=sim_time_s,
                    condition=condition,
                    C0=C0
                )
                ...
                ax1.plot(time_points / 60, concentrations, 'b-', linewidth=2.5)
                ...
                if half_life != float('inf') and half_life < sim_time_s:
                    ax1.axvline(half_life / 60, color='r', linestyle='--', linewidth=2,
                               label=f't½ = {half_life/60:.2f} min')
                    ax1.legend(fontsize=11)
                
                # Right plot: Conversion vs Time
                ax2.plot(time_points / 60, conversions_curve, 'g-', linewidth=2.5)
                ...
                # V3.4: Add decay info and reaction info at bottom
                decay_info = self.decay_kinetics.get_info_string(condition)
                info_text = f"T = {temperature-273.15:.0f}°C  |  Time: {sim_time_s/60:.1f} min  |  k₀ = {k:.2e} s⁻¹  |  Ea = {params['Ea']/1000:.1f} kJ/mol\n{decay_info}"
...
def main():
    ...
    print("\n" + "="*70)
    print(" 🧪 ENHANCED CHEMICAL REACTION & EDLC ANALYSIS SYSTEM V3.4")
    print(" BioCharge at CSUDH - Biochar Supercapacitor Research")
    print(" ✨ High-Throughput Platform V3.4 - Batch Processing Repaired")
    print("="*70)
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("\n📚 HELP - Version 3.4 Features:")
            print("\n🎯 WHAT'S NEW IN V3.4:")
            print("✅ Rate graphs now use MINUTES (more intuitive for short reactions)")
            print("✅ ChemPy & Chemlib integrated (enhanced chemical analysis)")
            print("✅ Bar charts instead of heatmaps (clearer metal comparisons)")
            print("✅ Manual SMILES input (no more dead ends for unknown compounds)")
            print("✅ Memory management (automatic garbage collection in batch mode)")
            print("✅ Exponential decay kinetics (realistic catalyst deactivation)")
            ...
            print("\n👋 Thank you for using the Enhanced Chemistry System V3.4")
